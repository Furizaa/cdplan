...
const SHARED = "Shared";
const TALENTS_DIRECTORY = "../db/talents";
    ...
    .map(
      (talent) =>
        `  ${talent}: { ${Object.keys(spellList[spec][talent])
          .map(
            (attribute) =>
              `${attribute}: ${JSON.stringify(spellList[spec][talent][attribute])}${
                (attribute === "id" && " as SpellId") || ""
              }`
          )
          .join(", ")} },`
    )
...
import { SpellList, SpellId } from '../types';
