...
CDPlan = LibStub("AceAddon-3.0"):NewAddon(CDPlan, "CDPlan", "AceConsole-3.0", "AceEvent-3.0")
LibRealmInfo = LibStub("LibRealmInfo")
LibSerialize = LibStub("LibSerialize")

-- Set up DataBroker for minimap button
SimcLDB = LibStub("LibDataBroker-1.1"):NewDataObject("CDPlan", {
  type = "data source",
  text = "CDPlan",
  label = "CDPlan",
  icon = "Interface\\AddOns\\CDPlan\\logo",
  OnClick = function()
    if SimcFrame and SimcFrame:IsShown() then
      SimcFrame:Hide()
    else
      CDPlan:PrintSimcProfile(false, false, false)
    end
  end,
...
local upgradeTable        = CDPlan.upgradeTable
local slotNames           = CDPlan.slotNames
local simcSlotNames       = CDPlan.simcSlotNames
local specNames           = CDPlan.SpecNames
local profNames           = CDPlan.ProfNames
local regionString        = CDPlan.RegionString
local zandalariLoaBuffs   = CDPlan.zandalariLoaBuffs
...
function CDPlan:OnInitialize()
  -- init databroker
  self.db = LibStub("AceDB-3.0"):New("CDPlanDB", {
    profile = {
      minimap = {
        hide = false,
      },
  ...
  LibDBIcon:Register("CDPlan", SimcLDB, self.db.profile.minimap)
  CDPlan:UpdateMinimapButton()

  CDPlan:RegisterChatCommand('cdplan', 'HandleChatCommand')
end

function CDPlan:OnEnable()

end

function CDPlan:OnDisable()

end

function CDPlan:UpdateMinimapButton()
  if (self.db.profile.minimap.hide) then
    LibDBIcon:Hide("CDPlan")
  else
    LibDBIcon:Show("CDPlan")
  end
end
...
function CDPlan:HandleChatCommand(input)
  ...
  for _, arg in ipairs(args) do
    ...
    elseif arg == 'minimap' then
      ...
      DEFAULT_CHAT_FRAME:AddMessage("CDPlan: Minimap button is now " .. (self.db.profile.minimap.hide and "hidden" or "shown"))
      CDPlan:UpdateMinimapButton()
      return
    end
  end
...
local function TranslateRole(spec_id, str)
  local spec_role = CDPlan.RoleTable[spec_id]
...
function CDPlan:GetItemStrings(debugOutput)
  local items = {}
...
function CDPlan:GetBagItemStrings()
  local bagItems = {}
...
function CDPlan:GetZandalariLoa()
...
function CDPlan:CovenantsAvailable()
  if Covenants then
    return true
  else
    return false
...
function CDPlan:GetActiveCovenantID()
  return Covenants.GetActiveCovenantID()
end
function CDPlan:GetActiveCovenantData()
  local activeCovenantID = CDPlan.GetActiveCovenantID()
  if activeCovenantID > 0 then
    return Covenants.GetCovenantData(CDPlan.GetActiveCovenantID())
  end
  return nil
end
function CDPlan:GetCovenantString()
  local covenantData = CDPlan.GetActiveCovenantData()
  if covenantData then
    return 'covenant=' .. CDPlan.covenants[covenantData.ID]
  end
  return nil
end
...
function CDPlan:GetSoulbindString(id)
...
function CDPlan:GetMainFrame(text)
...
local function recurseStringify(data, level, lines)
  for k, v in pairs(data) do
    ...
    if vType == "table" then
      ...
      table.insert(lines, strrep("    ", level) .. "},")
    else
...
function CDPlan.DataToString(data)
  if data then
    return CDPlan.SerializeTable(data):gsub("|", "||")
  end
end

function CDPlan.SerializeTable(data)
  local lines = {"{"}
...
function CDPlan:GetRoster()
  ...
  local outStr = CDPlan.DataToString(out);
  return outStr;
end

-- This is the workhorse function that constructs the profile
function CDPlan:PrintSimcProfile(debugOutput, noBags, showMerchant, links)
  -- addon metadata
  local versionComment = '# SimC Addon ' .. GetAddOnMetadata('CDPlan', 'Version')
  local simcVersionWarning = '# Requires CDPlan 901-01 or newer'
  ...
  CDPlanProfile = CDPlanProfile .. headerComment .. '\n'
  CDPlanProfile = CDPlanProfile .. versionComment .. '\n'
  CDPlanProfile = CDPlanProfile .. simcVersionWarning .. '\n'
  CDPlanProfile = CDPlanProfile .. '\n'

  CDPlanProfile = CDPlanProfile .. player .. '\n'
  CDPlanProfile = CDPlanProfile .. playerLevel .. '\n'
  CDPlanProfile = CDPlanProfile .. playerRace .. '\n'
  if isZandalariTroll then
    local zandalari_loa = CDPlan:GetZandalariLoa()
    if zandalari_loa then
      CDPlanProfile = CDPlanProfile .. "zandalari_loa=" .. zandalari_loa .. '\n'
    end
  end
  CDPlanProfile = CDPlanProfile .. playerRegion .. '\n'
  CDPlanProfile = CDPlanProfile .. playerRealm .. '\n'
  CDPlanProfile = CDPlanProfile .. playerRole .. '\n'
  CDPlanProfile = CDPlanProfile .. playerProfessions .. '\n'
  CDPlanProfile = CDPlanProfile .. playerTalents .. '\n'
  CDPlanProfile = CDPlanProfile .. playerSpec .. '\n'
  CDPlanProfile = CDPlanProfile .. '\n'

  -- SHADOWLANDS SYSTEMS
  -- gate covenant/soulbind code behind this check so addon will work in 8.3
  if CDPlan:CovenantsAvailable() then
    local covenantString = CDPlan:GetCovenantString()
    if covenantString then
      CDPlanProfile = CDPlanProfile .. covenantString .. '\n'
      ...
      if activeSoulbindID > 0 then
        local covenantsData = CDPlan:GetActiveCovenantData().soulbindIDs
        for _, soulbindID in pairs(covenantsData) do
          ...
          if soulbindData.unlocked then
            local soulbindString = CDPlan:GetSoulbindString(soulbindID)
            if soulbindID == activeSoulbindID then
              CDPlanProfile = CDPlanProfile .. soulbindString .. '\n'
            else
              CDPlanProfile = CDPlanProfile .. '# ' .. soulbindString .. '\n'
            end
          end
      ...
      CDPlanProfile = CDPlanProfile .. '# conduits_available=' .. conduitsAvailableStr .. '\n'
      ...
      if CovenantSanctumUI then
        ...
        if renown > 0 then
          CDPlanProfile = CDPlanProfile .. '# renown=' .. renown .. '\n'
        end
      end
  ...
  local items = CDPlan:GetItemStrings(debugOutput)

  -- output gear
  for slotNum=1, #slotNames do
    if items[slotNum] then
      CDPlanProfile = CDPlanProfile .. items[slotNum].string .. '\n'
    end
  end

  -- output gear from bags
  if noBags == false then
    local bagItems = CDPlan:GetBagItemStrings()

    if #bagItems > 0 then
      CDPlanProfile = CDPlanProfile .. '\n'
      CDPlanProfile = CDPlanProfile .. '### Gear from Bags\n'
      for i=1, #bagItems do
        CDPlanProfile = CDPlanProfile .. '#\n'
        if bagItems[i].name then
          CDPlanProfile = CDPlanProfile .. '# ' .. bagItems[i].name .. '\n'
        end
        CDPlanProfile = CDPlanProfile .. '# ' .. bagItems[i].string .. '\n'
      end
    end
  end
  ...
  if WeeklyRewards then
    if WeeklyRewards:HasAvailableRewards() then
      CDPlanProfile = CDPlanProfile .. '\n'
      CDPlanProfile = CDPlanProfile .. '### Weekly Reward Choices\n'
        ...
        for j, rewardInfo in ipairs(activityInfo.rewards) do
          ...
          if itemName then
            if itemEquipLoc ~= "" then
              local slotNum = CDPlan.invTypeToSlotNum[itemEquipLoc]
              CDPlanProfile = CDPlanProfile .. '#\n'
              CDPlanProfile = CDPlanProfile .. '# ' .. itemName .. '\n'
              CDPlanProfile = CDPlanProfile .. '# ' .. GetItemStringFromItemLink(slotNum, itemLink, nil, debugOutput) .. "\n"
            else
              ...
              if classId == 5 and subClassId == 2 then
                local level, _, _ = GetDetailedItemLevelInfo(itemLink)
                CDPlanProfile = CDPlanProfile .. '#\n'
                CDPlanProfile = CDPlanProfile .. '# ' .. itemName .. ' ' .. (level or '') .. '\n'
                CDPlanProfile = CDPlanProfile .. '# ' .. GetItemStringFromItemLink(nil, itemLink, nil, debugOutput) .. "\n"
              end
            end
          else
  ...
  if showMerchant and numMerchantItems > 0 then
    ...
    for i=1,numMerchantItems do
      ...
      if name and invType ~= "" then
        local slotNum = CDPlan.invTypeToSlotNum[invType]
        -- Doesn't work, seems to always return base item level
        -- local level, _, _ = GetDetailedItemLevelInfo(itemLink)
        CDPlanProfile = CDPlanProfile .. '#\n'
        CDPlanProfile = CDPlanProfile .. '# ' .. name .. '\n'
        CDPlanProfile = CDPlanProfile .. '# ' .. GetItemStringFromItemLink(slotNum, link, nil, false) .. "\n"
      end
    end
  end
  ...
  if links and #links > 0 then
    ...
    for i,v in pairs(links) do
      ...
      if name and invType ~= "" then
        ...
        CDPlanProfile = CDPlanProfile .. '#\n'
        CDPlanProfile = CDPlanProfile .. '# ' .. name .. '\n'
        CDPlanProfile = CDPlanProfile .. '# ' .. GetItemStringFromItemLink(slotNum, v, nil, debugOutput) .. "\n"
  ...
  local f = CDPlan:GetMainFrame(simcPrintError or CDPlan:GetRoster())
  f:Show()
end
